          ...
          ?.map((z) => {
            if (z?.totalWeight.length > 0) {
              ...
              if (z?.planType === 'Transitional') {
                ...
                return newData;
              } else if (z?.planType === 'Monthly' || z?.planType === 'monthly') {
                let extractString = z?.pouchesDetail;
                let newData = [];
                extractString.forEach((segment) => {
                  //console.log(segment);
                  let match = segment.match(/(\d+) servings x (\d+\d+) grams/);
        ...
        <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
          {BatchOrderByIdData?.map((t, firstindex) => {
            ...
            return FeedingData?.[firstindex]?.map((i) => {
              ...
              let newPouches =
                updatedPouches >= 400 && updatedPouches <= 800
                  ? updatedPouches / 2
                  : updatedPouches >= 800 && updatedPouches <= 1200
                  ? updatedPouches / 3
                  : updatedPouches >= 1200 && updatedPouches <= 1600
                  ? updatedPouches / 4
                  : updatedPouches;
              let numbers =
                updatedPouches >= 400 && updatedPouches <= 800
              ...
              return newArray?.map((x, thiredIndex) => {
                return (
                  <Grid item xs={2} sm={4} md={4} key={thiredIndex}>
                    <Card sx={{ width: 370, marginBottom: 3 }}>
                      <CardContent style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                        <Typography
                          style={{ marginTop: '16%', textAlign: 'center', height: 100, width: '20%' }}
                          variant="h5"
                          component="div"
                        >
